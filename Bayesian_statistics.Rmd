...
1. The more automated a task is, the less available it is to your conscious mind.

Our ability to verbalize our actions are limited. If there were no such verbalization limit, designing for tasks would be simple: just ask! Only mind-reading is more convenient. In user-centered design verbal inquiries are a central method. But, results fromn user interviews can be incomplete or biased. 

An example for incompleteness is that users sometimes fail to report what they think is obvious. For example, in modern computer desktop applications a function to undo a step is standard and probably it is one of the most frequently used functions. In other types of systems, for example data-base driven information systems, implementing an Undo function is non-trivial, because data base records can be changed by multiple users. It can be implemented by so-called roll-back mechanisms, which reduce performance and storage capacity of the system. When collecting user requirements for such a system, the developer has little interest in Undo, whereas the user has a huge interest, but does not mention it.

Verbal reports of action sequences are incomplete. But, if there is something even worse with humans: there ability to imagine a new situations. Imagine you had a time machine and you would travel back to the year 1981, which is a year before the series Knight Rider went on broadcast. If you would ask car drivers from that era, how they would envision a navigation assistant device, you would probably end up with something that looks more like a Star Trek computer console than with anything comparable to modern navigation device.

This why another central principle of user-centered design is observation of users. By directly observing how a person prepares a cup of coffee, we can learn about the details of behavior and close the verbalization gap. By observing many action sequences, we can carve out detailed user requirements, or fix an existing design. The first thing many people do in the mnorning is fix themselves a cup of coffee. With a conventional filter brewing machine, the user prepares the machine, switches it on and uses the time to release the bladder. Already in the bathroom, the user brushes teeth and the puts the laundry into the dryer. In the meantime, the coffee maker has done its job. Italian-style coffee makers need a period of pre-heating, before being operational. After the user has returned from the bathroom, the coffee maker is long back to sleep mode.

Observing user interactions does not require one-way mirrors and nights of video coding. Because of the verbalization gap, a lot can be learned just by watching over the users shoulder. The method of think-aloud usability testing even goes one step further and combines behavior observation with verbalization. Interaction sequences can even be collected without a human ovbserver. Log files of web servers provide sequences of users navigating web site. Plugin software is available that records keystrokes and mouse actions on computers. The difficult part is the following: When observing 50 users while doing a non-trivial task, no two interaction sequences are exactly the same (if i had to bet on it). By itself, there is little value without further means of interpretation and this can go two ways up: qualitative and quantitative.

The *qualitative design researcher* will collect interaction sequences, as well as verbal reports and shuffle them around until an intelligible picture emerges. Don't get me wrong: This is an art. One way to describe the purpose of user testing is to *find out all possible ways how things can go wrong.* Every break-down that is observed in only once in a sample of users, tells a story of what may happen to hundreds or thousands of users in the future.
...
ToT and other performance measures can be very noisy. The general recipe to reduce noise and improve certainty is to collect more data. The obvious way of doing this is inceasing teh sample size, but that can be costly. A more efficient way to reduce uncertainty with a given sample is to collect more data on every participant. It is therefore always recommended to plan for *repeated measures*. Chapter \@ref(mlm) will introduce models that deal with repeated measures in a straight-forward way.
...
Counting steps-to-completion or errors often requires the laborous process of interaction analysis. Measures of duration are much easier to obtain and in many situations they are spot-on: 
In usability studies, *time-on-task* is a direct measure of efficiency. When controlling a car in dense city traffic, a few hundred milliseconds is what makes huge a difference and therefore *reaction time* is a valid performance measure. In experimental cognitive research reaction times have been succesfully used in countless experiments to reveal the structure of the mind. 
...
The standard definition of usability defines three levels of usability, two performance-related effectiveness and efficiency, and satisfaction. The latter has long been held the enfant terrible of the three. That's not a big surprise, as satisfaction is about the *feelings* of users.
...
Feelings are a tricky business when it comes to measuring them. Most frequently, rating scales are employed, and often the same author makes the remark, that the study should be replicated with more solid measures for emotional states. Unfortunately those do not exist. Also, several authors took on the same construct, but called it differently, or the other way round.

Rating scales exist for a vast variety of concepts, but they come in only a few forms: first, there exist single item or multi-item scales. *Single-item* instruments are the hydrogen of rating scales. They are useful for obtaining a time-series of measures. For example, the effect of learning can be seen in a decline of cognitive workload over time. From a methodological point-of-view, single-item scales are to be seen critical, because many quality checks rest on having multiple items. Still, the validity of single-item scales can be proven by strict experiments.

*Multi-item* rating scales are the standard. Participants or users respond to a number of statements, that have an overarching theme. Multi-item scales are amenable to a number of procedures for quality check, such as reliability. In design research, measures of feelings are usually are taken as design benchmarks. It would be impractical to report responses of individual items. Therefore, multi-item ratings are routinely aggregated total scores, such as the *average score*.

Next to the number of items, rating scale instruments differ in *cardinality of the response*, how much possible responses there are. By far most rating scales use between four and nine *ordered bins*. So called *visual analog* rating scales measure on a  continuum. This can be done on paper, using a ruler to translate the response into numbers, or on teh computer, using a slider bar. That being said, one can also think of situations, where the answer is a simple Yes/No, such as for the question: Do you think you solved the task correctly?

Finally, rating scales have either *unipolar or bipolar anchoring* rating scales. Many rating scales put labels (instead of just numbers) on minimum and the maximum bins. With a unipolar item, the left anchor is neutral, whereas the right is not, like in:

0: Dull ... 1: Bliss

Dull is not the opposite of bliss, it is the absence of it, just like Zero is not the opposite of One. Bipolar scales are more heated than that, with hell and bliss:

-1: Ugly ... + 1: Beautiful

Another consideration regards binned bipolar scales. If such a scale has an uneven number, the center bin is neutral. 

-1:Ugly 0:Neutral +1:Beautifuly

Sometimes, this is a good thing, for example, when the participant may just not know:

The help pages of the program are written in a clear language.

-1: Not at all 0: I didn't notice +1: Totally

An even numbner of bins is recommended often for very long questionnaires, where there is the risk that participants automatically fink the neutral bin, because they are getting bored. In such a case, the absence of a neutral bin forces participants to make a choice.

What kind of rating scale you use has consequences for your statistical model. As we will see in section \@ref(psychometrics), multi-level models are a good match for multi-item rating scales. What really matters is whether using a visual analog scale or a binned scale. Data from visual analog scales is more easily treated, by either a Gaussian \@ref(psychometrics) or a Beta linearized model \@ref(beta-reg). Both are relatively lean methods. For binned rating scales you would have to use the beast called ordinal logistic regression \@ref(ord-logist-reg). My advice would be to use visual analog scales, whenever possible, even if this means caring a little less about exactly reproducing the original scale.
...
*Implicit measures* are means to assess certain attitudes or feelings by means of experimental setups. For example, We once tried to measure technophile attitude (geekism) using a variant of the classic Stroop task. In [%Schmettow, Noordzij, Mundt] we showed them pictures with computers on them, followed by a color-printed word (e.g., "explore") on which to perform the Stroop task. It was conceived that reaction time is increased when a participant experiences a strong association, like how good it feels to build your own computer. The initial success was soon washed away by a failure to reproduce these results. Another approach is the Approach-Avoidance task, which has gotten some merits in research on addiction and racism. In simple terms, participants (or users) are asked to push or pull a joystick and it seems that they pull faster, when they see something they like (a bottle of booze) and push faster when they dislike what they see (a photograph of war victims). However, I have seen this approach failing to produce relevant results in a design comparison experiment. Generally, such experiments produce reaction time differences below the 100ms mark and therefore many trials are needed to carve out any differences. At the same time, I have doubts that the emotional reactions towards, say computer interfaces, play in the same league as the stimuli used in research addiction or racism research. Emotional responses in design research may just be too mellow to disturb cognitive processes with a strength that is measurable. 
...
In this case, we can simply note down that the *most likely value* is $\pi_{Six} =.147$, which is lower than the fair `r round(1/6, 3)`. But, note the slight ruggedness of the bar chart. Not a single bar is read as exactly $1/6$, so the deviation of Six could have happened by chance. One way to approach this question is comparing the likelihoods $P(\text{Result = Six}|\pi = {1/6})$ and $P(\text{Result = Six}|\pi = {.147})$.
...
1. Giving priors an inferior role in this book was a pragmatic choice and this does not make me proud. For readers who want to pursue into this direction I  recommend 
    a. [%McElreath], who builds this topic from ground up and provides a user-friendly interface for the simultaneous specification of likelihood and prior. 
    a. [%BDA] is most explicit about prior specification, but requires mathematical skills.
1. Users who now feel that they thoroughly need to re-think Statistics, I warmly recommend  [%McElreath].
1. An extensive set of statistical distributions is contained by [%BDA]. Another good source on this topic is [%Wikipedia:stats_dist].
1. [%McElreath] uses a phantastic metaphor to explain teh MCMC algorithm.
...
Wait! We would have, if these studies  research had used New Statistics 
