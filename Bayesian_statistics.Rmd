...
### Likelihood of data {#likelihood}

[...]

Consider the following example: A standard dices is rolled twice. How likely is an outcome of two times Six? We use probability theory: It is the same dice and the results of the rolls are independent. The probability of rolling a Six on either attempt is 
$p(y_1 = \textrm{Six}) = p(y_2 = \textrm{Six})  = {1 \over 6}$. Therefore the joint probability is just the product:

$$
\pi(y_1 = \textrm{Six} \textrm{ and } y_2 = \textrm{Six}) \\= 
\pi(y_1 = \textrm{Six}) \times \pi(y_2 = \textrm{Six})= {1 \over 36}
$$
...
$$
\pi(y_1 = \textrm{Four} \textrm{ and } y_2 = \textrm{Five} \textrm{ and } y_3 = \textrm{Three} \textrm{ or } \textrm{Six}\\
=  {1 \over 6}
\times {1 \over 6} \times {1 \over 3} = {1 \over 108}
$$

Notice how the likelihood gets smaller in the second example. In fact, likelihoods are products of numbers between zero and one end therefore always become smaller, when another observation is added. In most empirical studies, the number of observations is much larger than two or three and the likelihood becomes inexpressible small. Consider the following following results from 16 rolls:
...
prob = 1/6
Likelihood <- prob^length(Result)
```



The likelihood of this result, given that it is a fair dice, is $6^{-16} = `r Likelihood`$. Usually, the *negative logarithm of the likelihood* is reported. This results in reasonably large positive numbers.

```{r}
(nlogLik <- -log(Likelihood))
```
...
```{r}
bind_rows(
  microbenchmark::microbenchmark(-prod(rep(1/6, 10000))),
  microbenchmark::microbenchmark(-sum(rep(log(1/6), 10000)))
)

```

The dice rolling example is a little odd. A visitor of a casino can expect that the probability of rolling a six is $1 \over 6$. A researcher would draw this into question: what is the most likely probability of rolling a Six? The following example is from a biased dice that has a probability of $1 \over 6$ to fall on One and a meager $1 \over 8$ for Six. Assume that the researcher knows that the dice *can* be biased, but not if and by how much.


```{r}
Events <- c("One", "One", "One", "Two", "Three", "Four", "Five", "Six")
(Result <- sample(Events, 50, replace = T))
```

The classic method of *maximum likelihood estimation (MLE)* can be used to answer the question. It makes use of the likelihood, by feeding it with proposed values for the unknown parameter $\pi(y_i = Six)$ and tries to find the point where the likelihood is at its maximum, where the observed data is most likely. In the most simple form, MLE starts at the left boundary, zero in this case, and walks in small steps along a grid to the right boundary (one):

```{r}

nlogLik <- 
  function(pi) -sum(if_else(Result == "Six", log(pi), log(1 - pi)))

Lik <- 
  data_frame(pi = seq(.01, .99, by = .01)) %>% 
  mutate(nlogLik = map_dbl(pi, nlogLik),
         rank = min_rank(nlogLik),
         MLE = (rank == 1))
Lik %>%          
  ggplot(aes(x = pi, y = nlogLik)) +
  geom_line() +
  geom_point(data = MLE, color = "Red") +
  geom_text( data = MLE, label = "MLE", color = "Red")

```

Because we use the *negative* log-liklihood, the value for $\pi$ with maximum likelihood is the minimum of the likelihood curve. Here, $pi = `r MLE[["pi]]` $ is considerably below the expected $1 \over 6$.

```{r}
(MLE <- filter(Lik, MLE))
```

In classic statistics MLE is one of the most common methods for estimating parameters from models. However, most of the time, data is more abundant and there is more than one parameter. It is possible tow extend the grid method to as many parameters as the model contains by just creating multi-dimensional grids. However, already a two-dimensional grid of rather coarse $100 \times 100$ would require the computation of 10.000 likelihoods. Classic statisticians have therefore developed several optimization methods to identify the MLE with as few runs as possible. Soon, we will turn our attention to Bayesian estimation, where the Likelihood plays a central role in estimation, too [REF]. Another appliction of the Likelihood is introduced in chapter \@ref(GOF), where it defines the deviance criterion for model goodness-of-fit.
