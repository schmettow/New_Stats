...
CE <- c("AUP", "BrowsingAB", "Chapter_LM", "Uncanny", "Hugme", "CUE8")
load_CE_(CE)
```
...
M_age_rtrn <- 
  stan_glm(returns ~ 1 + age_shft, data = BAB1, iter = 100)
...
```{r opts.label = "mcmc"}
M_bimod <- stan_glm(y ~ 1, data = D_bimod, iter = 100)
```
...
```{r opts.label = "mcmc"}
M_t <- stan_glm(y ~1, data = D_t, iter = 100)
...
In the BrowsingAB case, we have recorded age of participant and the LRM `M_age_shft` suggests that with every unit of age, average ToT increased by  `r C_age[2]` seconds. If we now ask: what is predicted for a person of age 45? We get the predicted value $\mu_i$:
...
```{r}
D_cross_valid <- sim_overfit(n = 100, seed = 1318)
...
Compared to LOO, the WAIC is blazing fast. However, the WAIC has two major down-sides: First, while the RMSE has a straight-forward interpretation as the standard deviation of the expected error, WAICs are  unintelligable by themselves. They only indicate relative predictive accuracy, when multiple models are compared. Second, WAIC as an approximation can be wrong. Fortunately, as can be seen from the warning above, the WAIC command performs an internal check on the integrity of the estimate. When in doubt, the function recommends to "run Loo instead".

The developers of the WAIC provide another IC, the LOO-IC. This is a more reliable approximation of the real LOO-CV, which is slower than WAIC, but still has reasonable computation times. The LOO-IC implementation also features an extra safety feature, by checking the integrity of results and helping the user to fix problems. 
...
The interpretation of LOO-IC is the same as for all information criteria, the value with the smaller IC wins. What often confuses users of information criteria is when they see two ICs that are huge with only a tiny difference, like 1001317 and 1001320. Recall that information criteria all depend on the likelihood, but in a logarithmic scale. What is a difference on the logarithmic scale is a multiplier on the original scale of the likelihood, which is a product of probabilities. And a small difference becomes a respectible multiplicator:
...
When the highly efficient WAIC reports unreliability, we traded some computing efficiency for reliability by switching too LOO-IC. If LOO-IC is unreliable for  a few observations, these can be fitted by running a few real LOO-CV with `reloo = T`. This can get time expensive with more than a few suffering obervations.  The authors of package Loo provide an alternative fall-back to reloo-ing: In *k-fold cross validation* a model is refit $k$ times, always leaving out and predicting $N/k$ observations. LOO cross validation actually is 1-fold cross validation, which results in fitting $N$ models. 10-fold cross validation stands on middle ground by training the model ten times on a different 90% part of data and testing the model on the remaining 10%:
...
Obviously, predictive accuracy can also be used to check for pover-dispersion, just compare, for instance, a Poisson model to  Negbinomial. I close this section with a more interesting comparison: In section [#dist_models] *distributional models* were introduced as a generalization  of GLMs, that allow to also link predictors to variance and shape parameters, such as the Gaussian $\sigma$, the Beta $\rho$ and the two parameters of the Exgaussian distribution, $\sigma$ for dispersion and $\beta$ for skew. When applied to the participant level, we saw a strong inter-individual variance in skew and variance, which seemed to justified the model. However, distributional models on participant-level are very opulent and the question is whether this model truly performs better in terms of predictive accuracy. 

```{r}
attach(Hugme)

loo_compare(loo(M_1_exg),
            loo(M_rt_dist)) 
...
For everyone who has to publish in Social Science journals, this book has so far left one question unanswered: What is the Bayesian method for testing null hypotheses? To give the short answer: There are no p-values in this statistical framework, but you can always use credibility limits to say "the effect is different from Zero with a certainty larger than 5%". If some very pertinent reviewers want p-values, you can argue that a null-hypothesis test is really just model selection, but unfortunately no p-value tests are available for the models that you are using, and then you introduce one of the methods based on predictive accuracy.
