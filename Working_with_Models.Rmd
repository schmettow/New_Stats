...
The least one can expect from a good model is that its predicted values resemble the shape of observed data. That is commonly called *goodness-of-fit*
...
UV_1 %>% 
  ...
  geom_smooth(aes(y = fitted), se = F) +
  geom_segment(aes(xend = huMech, yend = fitted), curvature = 0.1)
```
...
This allows us to easily create a combined summary based on the root mean square error:
