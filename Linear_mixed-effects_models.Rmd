...
Such a chance can count as good evidence in favor of Egan's claim, although it certainly does not match the "much more" in the original quote. However, if we take the strong and certain Item effect (Design x Task) into account, the claim could even be reversed (Table \@ref(tab:egan-5)). Apparently, the difficulty of a task depends on the design, that means, it depends on where this particular designer has placed an item in the navigation structure. These are clearly design choices and if we formulate it this way, the data provides very strong counter-evidence for Egan's claim:
...
P_1 %>% 
  ...
  kable(caption = "The probability that Egan's claim is right, also regarding the Item effects (Design x Task)")
```
...
With that many measures at our hand, how can we arrive at a single score to compare participants.  The approach known as *Classical Test Theory (CTT)* solves this by simply taking the average as a measure for processing speed (Table \@ref(tab:ctt-1)).
