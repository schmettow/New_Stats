...
CE <- c("BrowsingAB", "Typing", "CUE8", "Egan", "IPump")
load_CE_(CE)
```
...
```{r opts.label = "mcsync"}
sync_CE(BrowsingAB,M_1_SOV, P_1_SOV)
```
...
A chance of $`r C_Egan_is_right`$ can count as good evidence in favour of Egan's claim, although it is outshined by other factors, as we have discussed above. 
...
Under this perspective, we examine the data. This time, we have real time-on-task data and as so often, it is highly skewed. Again, we use the cheap trick of logarithmic transformation to obtain more symmetric distribution of residuals. The downside is that the outcome variable may not be zero. For time-on-task data this is not an issue. In fact, the original CUE8 data set contains several observations with unrealistically low times. Before proceeding to the model, exploring the original variable `ToT` on the two levels is in order. The mean ToT is computed for the two random effect levels, participants and teams and plotted in that order.
...
```{r opts.label = "mcsync"}
sync_CE(BrowsingAB,M_slope_1, P_slope_1)
```
