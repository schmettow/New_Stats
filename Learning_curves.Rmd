...
The third assumption is that every tweak has a certain probability to be found. This probability may vary between tweaks, but does not change over the process. A consequence of this assumption is that the learning process is *not* like climbing the stairs, but like finding Easter eggs. In the beginning there are many and you fill your basket in eggs per minute. Reportedly, finding the last Easter eggs can take a couple of months. For a beginning learner, performance improves very fast, because pool of tweaks is full and tweaks are found at a high rate. For an expert, the pool of tweaks has almost diminished, and it is hard to find new tweaks: performance improves at a slow rate.

When all tweaks are found and applied, the learning organism has reached its level of *maximum performance*. Maximum performance usually is a person-specific parameter that is always finite and never zero. It is never zero, because execution of complex tasks contains many functional areas of the executing organism, as well as physical constraints. If you learn to lace your shoes, many wondrous things are happening in your brain, but it won't grow the hands, or accelerate the travel time within nerve cells. We call this the *non-trainable part*.
...
1.  *Amplitude* reflects that at every point in the process there is a finite set of tweaks
2.  *Rate of survival* reflects the constant probability for a tweak to be found.
...
2.  Every tweak has a probability of $.002$ to be found, which means the *probability of survival* is $\textrm{Surv} = 1 - .002 = .998$
...
Since Surv is a proportion, it is a number, usually smaller than 1. this term has the property to get smaller to asymptotically (not eventually) reach zero:
