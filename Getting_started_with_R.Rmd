...
1. a model obkect holding the results  of a regression analysis
...
Of these four elements, three are pure text. For a minimal report it is a fairly convenient to  use a word processor software for the text, craft the figure in R and copy it. One problem with this approach is that a *scrunitable statistical report* contains at least the following elements:

1. data preparation
1. data exploration (ranges of variables, outlier discovery, visualizing associations etc.)
1. model estimation (formula specification, checking convergence)
1. model criticism (normality of residuals etc.)

In advanced statistical workflows this is then multiplied by the number of models an iterative selection process. Because it is easy to lie with statistics, these elements are needed as the build a fundament of credibility. A fully transparency is achieved, when another researcher can exactly reproduce all steps of the original analysis. One obvious way to achieve is to hand over the full R script. However, that is not particularly user-friendly. 


The most user-friendly way to achieve both: a good looking report and full transparency is to write a document that contains all before mentioned: text, graphics, tables and R code. In the R environment such mixed documents can be written in the *markdown/knitr* framework. Markdown implements a simple markup language, with that you can typset simple, structured  texts in a plain ASCII editor:
...
1. and if we can say that with sufficient uncertainty
...
The above text is an alternation of markup text and *chunks*, those weirdly decorated pieces of R code. When the document is processed by the knitr engine, the enclosed R code is evaluated and the output is woven into the document and per default, the R code is shown, too. Very conveniently, when the output is a figure, it will be inserted into the document. The `kable`  command from the knitr package produces neatly rendered tables from data frame objects.

A few basic markup tokens are used in the report:

+ \#, \#\# mark section headings on first and second level.
+ bullet points are marked by \+ at the beginning of the line
+ enumeration items are marked by *1.* at the beginning of the line
+ *italic* text is enclosed in \*...\*

There are more markup tokens in markdown and good documentation is available. The knitr engine parses the markup and translates everything into good looking HTML or Word documents and by intergration with the typesetting system LaTeX, professional quality PDFs can be produced, too. Still, for most of us mortals, the most pragmatic workflow is to create a comprehen
...
1. Do a full exploratory analysis of the dataframe D_agg in case environment IPump. It has the predictors Design, Group, Education and experience. It has the outcome variables ToT, deviations and workload.
    ...
    1. produce a table that counts the number of observations per Education
