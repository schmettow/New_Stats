...
Again, you have to do that only once after installing R and you can afterwards load the packages with the `library` command. Only if the package gets an update to add functionality or remove bugs, you need to run these commands again. <!-- #6 -->
...
Finally, R is a way to *develop and share statistical programs*. Thousands of packages in the R ecosystem cover any statistical problem you can imagine. Countless researchers and developers have written statistical routines that can be re-used. As a programming language, R has been designed for that particular purpose. Under the hood of R, a bunch of generic, yet powerful, principles purr to make it a convenient language for typical problems in statistical computation. Most of the people who are developing R have a background in general purpose programming languages, like Java, Python or C++.  <!-- #7 -->

*Functional programming* means that functions are first-class citizens in R. Most notably, almost every operation in R takes an input and creates an output object. In standard R programming there is nothing such as changing an object by setting its attributes. <!-- #8 --> Furthermore, there exist so called higher level functions. These are functions that apply other functions. For example, there is a function `aaply` in the `plyr` package, that applies another function, say `sum` per column or row of a matrix. These higher level functions allow for incredibly compact code and make loops practically unnecessary.
