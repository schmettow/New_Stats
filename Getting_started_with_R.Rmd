...
Most of the stuff that comes packaged with R we will set aside. While R at its very core is a brilliantly designed programming language for the task, much of the standard libraries is an inconsistent and outdated mess. For example, R comes with a set of commands to import data from various formats (SPSS, CSV, etc.). Some of these commands produce objects called data.frames, whereas others return list of lists. As another example, to sort a data frame by participant number, one has to write:

`D[order(D$Part),]`

In the past few years, a single man named Hadley Wickham started a movement that became known as the *tidyverse*. The tidyverse is a growing collection of libraries that ground on a coherent and powerful set of principles for data management. One of the core libraries is *dplyr* and it introduces some a rich, yet rather generic, set of commands for data manipulation. The above sorting of a data frame would be written as
...
Finally, R somes with a very rudimentary programming environment, with a charm of early eighties. Whereas a few alternatives exist, most R users will feel most comfortable with the R programming environment Rstudio. It is the most user-friendly and (at the same time!) feature richest software to program in R. Only some die-hard EMACS users might disagree.
...
This book is for applied researchers in design sciences, who's frequent task is to analyze data and report it to stakeholders. Consequently, the way I use R in this book capitalizes on interactive data analysis and reporting. As it turns out, a small fraction of R, mostly from the tidyverse, is sufficient to write R code that is effective and fully transparent. In most cases, a short chain of simple data transformations tidies the raw data and can readily be pushed into a modelling or graphics engine that will do the hard work. We will not bother (ourselves and others) with programming concepts such as conditionals or loops. At the same time, we can almost ignore all the clever and advanced routines that underlay statistical inference and production of graphics. Let me first give you a global account of programming in R, albeit that might be more interesting for readers who have experience with other programming languages (and styles). R serves three purposes: interactive data analysis, developing new statistical routines and reporting a data analysis. 

With R one typically *works interactively through a data analysis*. The analysis often is a rather repetitive series of steps, like: 
...
A program in R is usually developed iteratively: once you've loaded and checked your data, you progress to the next step step of your analysis, test it and proceed. At every step, one or more new objects are created in the environment, that capture intermediate and final results of the analysis:
...
R is a way to *report and archive* what precisely you have been doing with your data. In statistics, mathematical formulas  are the common form of unambiguously describing a regression model. For example, the following equation defines a linear regression model between the observed outcome $y$ and the predictor $x$:
