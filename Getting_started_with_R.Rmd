...
This book has been written in R. It is almost inevitable that the following introduction to R follows the style and workflow of when writing the book. A full treatment of R as a programming language, with all its features and idions is beyond the scope. However, the following introduction will present just a few concepts, which in the experience of the author (when writing this book), suffice for 90% of the tasks in data analysis.
...
This book is about pargramming and statistics at the same time. Unfortunatly, there are a few terms that have a particular meaning in both domains. One of those is "a variable". In order to avoid confusion and keep brevity, I am from now on referring to variables, when speaking of predictors and outcomes. For programming variables, I am using the term *object*, instead, although this is not strictly correct.


Objects can be of various *types*. R's most common basic types  are the following: 

* *logical*  objects for storing the two levels TRUE, FALSE, like presence or absence of a treatment, or passed and failed test items
* *factor* for categories or groups, like experimental treatments or participant IDs
* *integer* for discrete quantitative variables, like counts or ranks
* *numerical* for continuous quantitative variables, like most measures
* *character* for free text, for example, the description of a usability problem
...
Statistical analysis deals with real world data which ever so often is messy. Frequently, a planned observation could not be recorded, because the participant decided to quit or the equipment did not work porperly or the internet collapsed. Users of certain legacy statistics packages got used to coding missing observations as `-999` and then declared this the identifier for a missing value. In R missing values are first-class citizens. Every vector of a certain type can contain missing values, which are identified as `NA`. Most basic data manipulation functions, like `mean`
...
Most Behavioural research collects *real data* to work with. Like our simulation, real data usually contains several variables. In most cases, data is stored in a matrix-like structure, where columns are variables and rows are observations. R has *data frames* to store data. Data frames are tables, where columns represent statistical variables. Variables have names and can be  of different data types, as they usually appear in empirical research. You have already seen one ways of creating data frames. You create it by using the `data_frame` command, where you give alll the variable names and a vector with their values.

A data frame basically is a set of variables in columns and values in rows. The variables can be of different types, like factors or numbers. Data frames are objects, too. As we have seen above, you can print a data frame to the screen by just calling teh object by name.
...
If this is too much information, several commands are available to look into a data frame. The `str` (structure) command works on any R object and gives quite a useful output to understand  a data frame:

```{r opts.label = "rtut"}
str(Experiment)
```

Simple descriptives statistics for all variables (i.e. columns) are given by the `summary` command.

```{r opts.label = "rtut"}
summary(Experiment)
```

A third option many people use is to print just the first few rows of a data set:

```{r opts.label = "rtut"}
head(Experiment)
```

Often, you want to assess a single variable from your data frame. This can be done with the `$` operator. For example, the following command calculates the mean outcome:
...
While `data_frame[]` behaves perfectly predictable in terms of the returned object, extracting a variable as a vector is sometimes neded. With a `data_frame` a single column can be extracted using double square brackets, or using the `$` operator. Both variants will always return a vector.
...
In the book package (directory `/Data`) you will find the data set of the (virtual) `` study BAB1, which we will be using in coming chapters. This data comes as text file with the file ending `.csv`. Load this file into R using the `read.csv` command. 


Exercise 2:

1. drop an Excel and/or an SPSS file into the same directory as your current R file. 
2. Read the data into a data frame 
3. summarize what is in the data frame


### Manipulating data

Do you wonder about the strange use of `%>%` in my code above? This is a modern way of programming in R, that grounds on the [split, apply, combine](http://zevross.com/blog/2015/01/13/a-new-data-processing-workflow-for-r-dplyr-magrittr-tidyr-ggplot2/) paradigm, which is famous in big data  scenarios. 
...
Finally, for the descriptive statistics part of your report, you probably want to summarize the outcome variable by condition. The following chain of commands first group the dataframe, then computes means and standard deviations. At every step, a data frame is piped into another command, which processes the data frame and outputs a data frame.
