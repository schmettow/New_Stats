...
Most  complex routines in R have an abundance of arguments, most of which have reasonable defaults. To give a more complex example, the following call of `stan_glm` performs a Bayesian estimation of a grand mean model. The required arguments of `stan_glm` are the model and the data. The model is given as a formula and data is expected to be a dataframe. By default this will run the model on the data, with a Normal random component, with three chains and with 1000 iterations per chain.

The second invocation of `stan_glm` changes the defaults of two arguments. The same model is estimated, but with a Poisson random component and only 500 iterations are done per chain. (What that precisely means will be explained in Part 2.) <!-- #10 -->
...
Note that working with environments is a tricky business. Creating these case environments in an efficient way was more difficult than you may think. Therefore, I do *not* recommend using environments in everday data analysis, with one exception: if you are working on multiple projects simultaneously, because at any moment the current working environment contains all objects that have been created, so far. That is precisely the set of objects shown in the Environment pane of Rstudio (or call `ls()` for a listing). Another way of dealing with this is saving all objects and retrieving them when returning from a break or switching projects, it is as easy as:<!-- #13 -->
