...
In advanced statistical workflows this is then multiplied by the number of models an iterative selection process. Because it is easy to lie with statistics, these elements are needed as the build a fundament of credibility. A fully transparency is achieved, when another researcher can exactly reproduce all steps of the original analysis. It is obvious that the easiest way to achieve is to hand over the full R script.
...
![A minimal statistical report in markdown](figures/markup_minimal_report.png)


The above text is an alternation of markup text and *chunks*, those  weirdly enclosed pieces of R code. While the text is static, the chunks are processed by the knitr engine, evaluating the enclosed R code and knitting the the output into the document. Very conveniently, when the output is a figure, it will be inserted into the document, right away. The `kable`  command from the knitr package, in turn, produces neatly rendered tables from data frame objects. By default, the R code is shown, too, but that can be customized. 

The minimal workflow for statistical reporting with knitr is as follows:

1. Use markdown right away, covering all steps of your data analysis, i.e. a scrunitable report. You may even start writing when only one part of your data gathering is completed, because due to the dynamic chunks. updating the report when new data arrives is just a button click away.
1. When the data analysis is complete, compile the scrunitable report to Word format
1. extract the passages, figures and tables for a minimal statistical report. This is your results section.
1. provide the scrunitable report as appendix or supplementary material
