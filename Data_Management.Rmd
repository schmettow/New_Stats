...
Ddm = list() # data sets
Mdm = list() # models
Sdm = list() # simulation functions
Tdm = list() # tables
load("Data_Management.Rda")
```
...
```{r read_data, opts.label = "invisible"}
setwd("data/")
...
Ddm[["HUG3_exp"]] <- HUG3 %>% 
  select(Participant = Subj,
         Trial, Target, targetCat = HUG, Prime, primeCat, RT, correct)

Ddm[["HUG3_resp"]] <- 
  Q3 %>% 
  select(Participant = Subj, Geek01:GeekR34, NCS01:NCS11) %>% 
  gather(Item, rating, -Participant) %>% 
  ...
  arrange(Participant, Scale, Item)

Ddm[["HUG3_part"]] <- Q3 %>% 
  select(Participant = Subj, gender = Gender, age = Age)
...
```{r tab:shopping_cart_wide_table, opts.label = "fig.small"}
...
![Entity relationship model of a shopping cart](figures/DM/erm_1.png)

## Modelling questionnaire responses
...
```{r fig:questionnaire_entity, opts.label = "fig.wide"}
Tdm[["HUG3_some_items"]] <-
  Ddm[["HUG3_resp"]] %>% 
  select(Item, Scale) %>% 
  distinct() %>% 
...
In more complicated cases, the combination of participant and item might not identify every observation uniquely. For example, a preferred method to assess a questionnaires reliability is to repeat the measurement in a second session. If we would use the item table as in `r figr("fig:questionnaire_entity")`, the observations were no longer uniquely identified, as all participants encounter every item twice. As a solution one would simply extend the key of the table by a variable called `session`.
...
```{r fig:experiment_exchangeable_data_model, opts.label = "fig.wide"}
Tdm[["exchangeable_between_subj_part"]] <-
  data_frame(Participant = as.factor(c(1:3)),
             gender = c("f","m","m"),
             age = c(23,22,26),
             group = as.factor(c("gamer", "gamer", "no_gamer")))

Tdm[["exchangeable_between_subj_exp"]] <-
  expand.grid(Participant = as.factor(c(1:3)),
              position = 1:3) %>% 
  mutate(waiting_time = runif(9, 10, 60),
         RT = rexgauss(9, 200, 20, 100))

Pdm[["exchangeable_exp_data_model"]] <- 
  arrangeGrob(Tdm[["exchangeable_between_subj_part"]] %>% gtable(plot = F),
             Tdm[["exchangeable_between_subj_exp"]] %>% gtable(plot = F),
             ncol = 2)
plot(Pdm[["exchangeable_exp_data_model"]])

```


The model gets just slightly more complex when the experiment comprises a condition. Assume an experiment that examines the difference in vigilance between gamers and non-gamers. As this is a between-subject design, the condition is assigned to the participant table, as is shown in `r figr("fig:experiment_exchangeable_data_model")`. Note that the experiment table only contains the absolute necessary attributes, namely all information that characterizes the encounter itself. Of course, for the data analysis we need a table that contains information on participants as well. A later section will show how to combine and manipulate data from separate tables.

```{r fig:experiment_Stroop, opts.label = "fig.wide"}
Tdm[["Stroop_part"]] <-
  data_frame(Participant = as.factor(c(1:3)),
             gender = c("f","m","m"),
             age = c(23,22,26))
...
Tdm[["Stroop_stim"]] <-
  data_frame(Stimulus = as.factor(1:4),
...
Tdm[["Stroop_exp"]] <-
  expand.grid(Participant = as.factor(c(1:3)),
              Stimulus = as.factor(c(1:4))) %>% 
  mutate(    RT = rexgauss(n(), 200, 20, 100 )) %>% 
  arrange(Participant) %>% 
  slice(1:9)

Pdm[["Stroop_data_model"]] <-
arrangeGrob(
  Tdm[["Stroop_part"]] %>% gtable(plot = F),
  Tdm[["Stroop_stim"]] %>% gtable(plot = F),
  Tdm[["Stroop_exp"]] %>% gtable(plot = F),
  layout_matrix = rbind(c(1,3), c(2,3))
)

plot(Pdm[["Stroop_data_model"]])
...
```{r fig:HUG3_experiment_data_model, opts.label = "fig.wide"}
Tdm[["SemStroop_part"]] <-
  data_frame(Participant = as.factor(c(1:3)),
             gender = c("f","m","m"),
             age = c(23,22,26),
...
Tdm[["SemStroop_stim"]] <-
expand.grid(Prime = unique(Tdm[["SemStroop_prime"]]$Prime),
            Word_EN = unique(Tdm[["SemStroop_word"]]$Word_EN),
            stringsAsFactors = F) %>% 
  slice(c(1,6,9,13)) %>% 
#   left_join(Tdm[["SemStroop_prime"]]) %>% 
#   left_join(Tdm[["SemStroop_word"]]) %>% 
  mutate(Stimulus = as.factor(row_number())) %>% 
  select(3,1,2)

Tdm[["SemStroop_exp"]] <-
  expand.grid(Participant = as.factor(c(1:3)),
              Stimulus = as.factor(1:4)) %>% 
  arrange(Participant) %>% 
...
grid.arrange(
  Tdm[["SemStroop_part"]] %>% gtable(plot = F),
  Tdm[["SemStroop_word"]] %>% gtable(plot = F),
  Tdm[["SemStroop_prime"]] %>% gtable(plot = F),
  Tdm[["SemStroop_stim"]] %>% gtable(plot = F),
  Tdm[["SemStroop_exp"]] %>% gtable(plot = F),
...
![Entity relationship model of a hybrid survey/experimental study](figures/DM/erm_2.png)
...
```{r old_way_one_liner, opts.label = "rtut.nr"}
...
```{r new_way_one_liner, opts.label = "rtut.nr"}
...
```{r read_UC_resp, opts.label = "rtut"}
setwd("data/UC2/")
## reading EXcel
UC_resp <-
...
```{r read_UC_resp_CSV, opts.label = "rtut"}
setwd("data/UC2/")
## reading CSV
UC_resp <- 
...
```{r read_UC_part_sav, opts.label = "rtut"}
setwd("data/UC2/")
## reading SPSS
UC_demo <- 
  read_sav("UC_part.sav") %>% 
...
```{r load_raw_experimental, opts.label = "rtut"}
setwd("data/UC2/")
...
```{r process_questionnaire, opts.label = "rtut"}
UC_scores <- 
  UC_resp %>% 
...
```{r UC_join_exp_resp, opts.label = "rtut"}
UC_1 <- 
  UC_exp %>% 
...
```{r fig:latent_variable_models, opts.label = "fig.small", echo = F}
DiagrammeR::grViz('
   digraph rmarkdown {
...
opts_knit$set(root.dir = thisdir)
```
...
setwd(thisdir)
Ddm$UC_exp <- UC_exp
Ddm$UC_scores <- UC_scores
# Ddm$UC_demo <- UC_demo
Ddm$UC_1 <- UC_1

save(Ddm, Pdm, Mdm, Sdm, Tdm, file = "Data management.Rda")
