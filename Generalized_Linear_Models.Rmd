...
Further down, we will take the linear model formula literally and discover what happens when we simulate from it. The simulations will raise skepticism. But, it is real data that leaves no doubt the linear model is far off when it comes to typical measures in design research. For the first time 
...
There is a conflict between the extreme versatility of the linear term, and its unfortunate assumption of linearity. Generalized linear models use a simple mathematical trick to keep the linear term, without permitting the expected values to ever cross the boundaries.
...
Some researchers apply transformations to their variables, to make them meet the assumptions. This may look similar, but it is not, as the sole purpose is to squeeze the distribution of data into a form that is "more normal". It is a gamble, sometimes it works, sometimes it does not. Even worse, it renders the linear coefficients useless. In quantitative design research, a statement like the following has no reasonable, let alone natural, interpretation: interface A can be use 2 *log-seconds* better than B. 

If one realizes that distributions in real data are rarely normal, it is getter to go with the real thing, which 
