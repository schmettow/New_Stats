...
### Re-linking linearity (#relinking_linearity)
...
Note that in order to account for over-dispersion, observation-level random effect (`1|Obs`) has been used, see [overdispersion]. For the current matter, we only need to inspect population-level coefficients:
...
#### Successes in a number of trials
...
+ In psychometrics, the famous Rasch model applies for measuring a persons ability by teh number of correct answers in a test. A Rasch model is just a cross-classified multilevel logistic regression [designometrix].
...
+ In section [ordered_lr], we will encounter *ordinal logistic regression*, which applies for classifications with an order, such as responses on Likert scales.
...
Beta-binomial regression follows a similar pattern as neg-binomial. A two parameter distribution allows to scale up the variance relative to a binomial model [#logistic_regression]. A beta-binomial distribution is created by replacing binomial parameter $p$ by a $beta distribution$, with parameters $a$ and $b$:
...
## Duration measures {#measures_of_time}
...
*Note* that the Gamma model caused trouble when estimated with an identity link. For this reason, all three models were estimated on a log-linearized scale. This makes the back-transformed coefficients multiplicative, which actually makes more sense, as we have seen in [#speakin multiplicative]. 
...
### Participant-level skew in reaction times
...
That does not mean the distributional model is a fruitless endeavour. In the following chapter, we will learn how to compare models by predictive accuracy. As it will turn out in [choosing_distributions], the distributional Beta model in the Hugme case has a better predictive accuracy than the location-only model. The same is true for the Exgaussian distributional model and left to the reader as an exercise.
