...
Poisson and binomial distributions are one-parameter distributions, with the  effect that location and dispersion are tightly entangled. 
...
to some other distribution. Under this perspective, the negative binomial distribution is equivalent to a Poisson distribution, where the parameter $\lambda$ follows a certain gamma distribution:
...
For beta-binomial distributions, the mixture is even more straight forward:
...
The brms regression engine implements both response distributions for overdispersed count data. Both families, negative binomial and beta-binomial are repqrq 
