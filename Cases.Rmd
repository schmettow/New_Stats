...
print.tbl_obs <- function(x, n = 8) {
  cap <- str_c("showing ", n, " of ", nrow(x), " observations")
  tab <- dplyr::sample_n(x, n)
  if("Obs" %in% colnames(tab)) tab <- arrange(tab, Obs)
  if("Part" %in% colnames(tab)) tab <- arrange(tab, Part)
  tab <- kable(tab, align = "c") %>% 
