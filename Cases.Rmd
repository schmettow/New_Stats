...
#          Stimulus = str_remove(Stimulus, "^c"),
#          Experiment = if_else(Experiment == "Keeris", "DK", "MR"))
...
Uncanny$PS_1 <- 
  ...
  ungroup() %>% 
  select(Part, Item, Stimulus, )

Uncanny$RK_1 <-
  RK_1 %>% 
  mutate(Experiment = "RK",
         Collection = "RK",
         Condition = "long",
         Session = as.character(repetition + 1)) %>%
  group_by(Part) %>%
  mutate(trial = row_number()) %>% 
  ungroup()
...
# Uncanny$D_all <-
#   MathurRepl %>% 
#   bind_rows(Uncanny$PS_1 %>% select_(.dots = names(PS_))) %>% 
#   bind_rows(Uncanny$RK_1 %>% select_(.dots = names(MathurRepl))) %>% 
#   as_tbl_obs()
...
## IPump: Comparing performance over time between two infusion pump designs 
