...
These probabilities we can feed into the decision framework as outlined above. The problem is, that we obtained just a few obervations to infer the magnitude of the parameter $P(rain|cloudy) = `r T_desc[2,2] * 100`$%. Imagine, you would repeat the observation series on another 20 days. Due to random fluctuations, you would get a more or less different series and different estimates for the probability of rain. More generally, the *true* parameter is only imperfectly represented by any sample, it is not unlikely, that it is close to the estimate, but it could be somewhere else, for example, $P(rain|cloudy) = `r jitter(as.numeric(T_desc[2,2]), 5) * 100`$%. 
...
The results tell her that participants spend $`r T_fixef$center[1]`$ Euro on average with the current design. The prototype seems to increase the revenue per transaction by $`r T_fixef$center[2]`$. That seems sufficient, but uncertainty is strong, too, as indicated by the 95% credibility intervals. There is a chance of more than 2.5%, that the revenue is just $`r T_fixef$lower`$ (or lower), such that the target is not met. But what precisely is the chance of falling below EUR 5? In Bayesian analysis we usually get the complete posterior distribution, which we can evaluate to get a definite answer:
...
The red area represents the danger zone, where revenue increase does not reach  EUR 5. The area amounts to `r N_risk_of_failure_1 * 100`% of the total probability. With this information the manager now has several options:

1) decides that `r N_risk_of_failure_1 * 100`% is a risk she *dares* to take
...
$`r T_fixef[[1,2]]`$. That is not very promising. In fact, the odds that the average user can do it in 99 seconds is a feeble `r round(N_99s[1] * 100, 0)`%. Luckily, Jane had the idea that the slogan could be changed to *"rent a card in 1-1-1 seconds"*. The certainty that this slogan holds is a favorable `r round(N_111s[1] * 100, 0)`, a risk that a brave marketing department is surely willing to take.
