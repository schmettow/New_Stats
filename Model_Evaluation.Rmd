...
This model does not take any predictors into account and therefore, in this special case, the intercept is the only prediction.
...
do_loo <- function(data, f_fit, f_predict){ 
  # data <- Ver20
  # f_fit <- fit_M_1
  leave_out <- function(obs) slice(data,-obs)
  left_out  <- function(obs) slice(data, obs)
  f_predict <- function(fit, obs) predict(fit, newdata = obs)$center[1]
  
  out <- data_frame(obs     = 1:nrow(data),
                    Sample  = map(obs, leave_out),
                    Obs     = map(obs, left_out),
                    Fitted  = map(Sample, f_fit)) %>% 
    mutate(forecast = unlist(map2(Fitted, Obs, f_predict)))
  return(out)
}

Loo_1 <- do_loo(D, fit_M_1)
...
Models are all about their predictive ability. Predictive ability seems to imply that the data must speak about the furture, which sounds like gathering more data. In fact, it suffices to have just *out-of-samnple data*, not true forecasting.
