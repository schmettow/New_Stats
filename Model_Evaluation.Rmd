...
CE <- c("Uncanny", "BrowsingAB", "CUE8", "Hugme", "Stroop_comp", "IPump", "Sec99", "Stroop", "StroopMW", "Chapter_WM")
load_CE_(CE)
...
1. When the main purpose is predicting future observations with optimal accuracy, *model pruning* is a procedure for selecting a subset of parameters with the optimal forecasting accuracy.
...
1. Measure the forecasting accuracy for $i$.
...
1. Other than one might expect, the function does not return a single score for forecasting accuracy, but a dataframe with inidividual forecasts. This is on purpose as there is more than one possible function to choose from for calculating a single accuracy score.
...
Note, how the output of the three runs of `do_loo` are simply merged into one dataframe.
This allows us to create a model comparison summary based on the root mean square error:
