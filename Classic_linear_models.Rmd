...
The regression object stores the MCMC results as a long series of positions in parameter space. For any range of interest, it is the relative frequency of visits that represents its certainty. The first 50 hundred steps of the MCMC random walk are shown in \@ref(fig:99_seconds_random_walk)`. Apparently, the random walk is not fully random, as the point cloud is more dense in the center area. This is where the more probable parameter values lie. One can clearly see how the MCMC algorithm jumps to more likely areas more frequently. These areas become more dense and, finally, the cloud  of visits will approach the contour density plot above. 
...
In our example, in \@ref(99_seconds_post) we can spot that the most likely value for average time-on-task is  `r frm_coef(Sec99$T_coef_1, row = 1, interval = F)`. Both distributions have a certain spread. With a wider PD, far-off values have been visited by the MCMC chain more frequently. The probability mass is more evenly distributed and there is less certainty for the parameter to fall in the central region. In the current case, a risk averse decision maker would maybe take the interval  $`r md_coef(Sec99$T_coef_1, ~fixef == "Intercept", center = F)`$ as "reasonably certain".
...
Pearson $r$ basically is a *standardized covariance* and a covariance is by itself a measure of association between two variables X and Y, namely:
...
In plain words, the covariance formula takes the deviations of both variables from their respective expected value and multiplies them. When positive deviations of X coincide with positive deviations of Y, that product gets larger; the same for negative values. The average of the deviation products is the covariance.

The difference between an observation and the mean of observations is nothing else than a center shift transformation. However, the scale of the variables stays. In effect, covariances can hardly be interpreted by themselves and two covariances can only be compared, if they share the scale of measurement. If one wants to compare the association strength of two predictors, say age and number of previous uses on ToT, that is inconvenient.

Pearson $r$ solves the problem by rescaling covariance using the standard deviations of X and Y:
...
```{r}
cov(D_cor$x, D_cor$y)/(sd(D_cor$x) * sd(D_cor$y))
cor(D_cor$x, D_cor$y)
```


Due to the full standardization, $r$ lets the researcher interpret strength of association independent of scale of measurement. More precisely, $r$ will always sit in the interval $[-1,1]$. That makes $r$ a perfect choice for many purposes. For example, in psychometrics the reliability of a measure is often assessed in test-retest settings. All participants undergo two separate tests and the correlation between the two scores is a measure for stability of the test, also called test-retest reliability.

The slope parameter in an LRM is a measure of association, too. However, it fully remains on the scale of measurement. In the context of quantitative assessment we used to see that as an advantagebecause it allows us to make quantitative statements and predict. Still, one can obtain Pearson $r$ easily by rescaling the slope coefficient $\beta_1$  using the following formula:

$$
r = \beta_1 \frac{sd_X}{sd_Y}
$$

For the purpose of demonstration, we here resort to the built-in non-Bayesian command `lm` for doing the regression.
...
Pearson's $r$ spawns from a different school of thinking than quantitative Bayesian regression: analysis of variance (ANOVA). Roughly, this family of methods draws on the idea of dividing the *total variance* of the outcome variable into two components: *explained variance* and *residual variance*. The very formula the variance statistic reveals the origins (it is even allowed to say, that variance is the covariance of a variable with itself):

$$
\textrm{Var}_{X} = \frac{1}{n} \sum_{i=1}^n (x_i - E(X))^2
$$
In ANOVA models, when explained variance is large compared to residual variance, the F-statistic goes up and stars twinkle behind the p-value. While I am far promoting any legacy approaches, here, a scale-less measure of association strength can be useful in situations, where at least one of the involved variables has no well-defined scale. That is in particular the case, when measuring with rating scales. Measurement theory tells that we may actually transform rating scales fully to our liking, if the order is preserved. That is a pretty weak criterion and, strictly speaking, forbids the application of linear models (and ANOVA) altogether, where at least sums must be well defined.
...
When using the two predictors simultaneously, the overall positive tendency remains. However, we observe major and minor shifts: in the MRM, the strength of the geekism score is reduced to less than half: `r frm_coef(AUP$T_coef_3, ~model == "M_3", ~fixef == "zgex")`. NCS has shifted, too, but lost only little of its original strength: `r frm_coef(AUP$T_coef_3, ~model == "M_3", ~fixef == "zncs")`. 
...
The reason is that the two *predictors are correlated*. In this study, participants who are high on NCS also tend to have more pronounced geekism. \@ref(fig:AUP_corr_predictors) reveals the situation:
...
```{r opts.label = "mcmc"}
M_1 <- stan_glm(y ~ 1, data = D_1, iter = 200)
```
...
```{r}
set.seed(2)
D_2 <- data_frame(y = rt(200, 2))
```


```{r opts.label = "mcmc"}
M_2 <- stan_glm(y ~1, data = D_2, iter = 200)
...
D_2 <-   mutate(D_2, resid = residuals(M_2))
C_sigma <- rstanarm::sigma(M_2)
...
D_QQ <- data_frame(step = 0:100/100,
           quant_smpl = quantile(D_2$resid, step),
...
## Welcome to the rollercoaster: polynomial regression [EDIT]
...
```{r trough_CI95}
quantile(P_1_trough, c(.025, 0.5, .975), na.rm = T) %>% 
  kable()
```
