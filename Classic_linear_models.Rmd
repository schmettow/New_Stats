...
Due to the full standardization, $r$ lets the researcher interpret strength of association independent of scale of measurement. More precisely, $r$ will always sit in the interval $[-1,1]$. That makes $r$ a perfect choice for many purposes. For example, in psychometrics the reliability of a measure is often assessed in test-retest settings. All participants undergo two separate tests and the correlation between the two scores is a measure for stability of the test, also called test-retest reliability. By using correlations, psychometricians can set absolute standards for the reliability of assessments.

The slope parameter in an LRM is a measure of association, too. However, it fully remains on the scale of measurement. In the context of quantitative assessment we used to see that as an advantage, because it allows us to make quantitative statements and predict future outcomes. Still, one can easily  obtain Pearson $r$ by rescaling the slope coefficient $\beta_1$  using the following formula:
...
By adding gender to the model, both effects are estimated simultaneously. In a *factorial MPM* the intercept is a reference group, too. Consider that both factors have two levels, forming a $2x 2$ design with groups: A-F, A-M, B-F, B-M. The first one, A-F, has been set as reference group. Women in condition A have an average ToT of `r frm_coef(T_fixef_mpm_1, ~fixef == "Intercept")` seconds.
...
The intercept still is the performance of an average twen using reference design A. Compared to the MPM, it is less favorable, with `r frm_coef(T_ia1, ~model == "M_ia1", ~fixef == "Intercept")` seconds time-on-task. The effect of design B improved dramatically with the DDM: a twen is now `r frm_coef(T_ia1, ~model == "M_ia1", ~fixef == "DesignB")` faster with it. Is B the clear winner? It is not,  because A has much lower age effect. The age parameter does no longer represent both groups, but just reference A, and it is now very small, `r frm_coef(T_ia1, ~model == "M_ia1", ~fixef == "age_shft", interval = T)`. The final coefficient is *not* the age effect in B, but *the difference* to the reference age effect. With design B, users are getting `r C_ia1_ageB` seconds slower per year of life. That is a lot!
...
The model has factors only, so there is a reference group. The intercepts in both models represent low education encountering design A. The main  effect designB on low-educated  users is present in both models. With the interaction term, design B looks much favorable, `r frm_coef(T_ia2, ~model == "M_ia2", ~fixef == "DesignB")`. The effect of middle education has doubled, whereas it remains stable for high education. 
...
$$\mu_{12pt, black} = `r T_read_fixef[1,3]` + `r T_read_fixef[2,3]` + `r T_read_fixef[3,3]` + `r T_read_fixef[4,3]` = `r sum(T_read_fixef[,3])`$$
