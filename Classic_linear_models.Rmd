...
CE <- 
  c("BrowsingAB", "Sec99", "AUP", "MMN", "Headache", "Reading", 
    "AR_game", "Sleep", "Uncanny", "Chapter_LM", "IPump")
load_CE_(CE)
...
D_ratings <- 
  D_ratings %>%
  gather(key = Item, value = rating, difficult_easy:neutral_uncanny, ) %>% 
...
```{r}
D_ratings %>% 
  mutate(zdifficult_easy = (difficult_easy - mean(difficult_easy))/sd(difficult_easy)) %>% 
  mascutils::z_score(hell_heavenly:neutral_uncanny) %>% 
  gather(key = Item, value = rating, -Part) %>% 
  filter(str_detect(Item, "^z")) %>% 
  ggplot(aes(x = rating)) +
  facet_grid(Item ~ .) +
  geom_histogram(bins = 100)
```
...
In the BrowsingAB case, we have repeatedly recorded age of participant. The LRM `M_age_shft` found that with every unit of age, average ToT increased by  `r C_age[2]` seconds. If we now ask: what is predicted for a person of age 45? We get the predicted value $\mu_i$:
