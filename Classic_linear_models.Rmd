...
Setting $\beta_1 = 0$ produces a horizontal line, with $y_i$  being constant over the whole range. This shows that The linear regression model (LRM) can be conceived a generalization of the GM model: $\mu_i = \beta_0$.
...
Most rating scales are not natural units of measure. Most of the time it is not  meaningful to say: "the average user experience rating improved by one". The problem has two roots, as I will illustrate by the following four rating scale items:
...
D_ratings <-
  tibble(Part = 1:100,
         difficult_easy  = mascutils::rrating_scale(100, 0, .5, 
                                                    ends = c(1,7)),
         hell_heavenly   = mascutils::rrating_scale(100, 0, .2, 
                                                    ends = c(0,10), bin = F),
...
The first problem is that rating scales have been designed with different end points. The first step when using different rating scales is to adjusted for that by shifting the left-end point to zero and downscaling by the (new) right-end point:

```{r}
D_ratings <-  D_ratings %>%
  mutate(difficult_easy  = (difficult_easy - 1) /(7 - 1),
         hell_heavenly   = hell_heavenly/10,
         neutral_uncanny = (neutral_uncanny - 1)/(5 - 1))
...
This partly corrects the horizontal shift between scales. However, the ratings on the third item still are shifted relative to the other two. The reason is that the first two items have the neutral zone right in the center, whereas the third item is netraul at its left-end point. The second inconsistency is that the second item uses rather extreme anchors (end point labels), which produces a tight accumulation in the center of the range (with a lot of polite people in the sample, at least).

These two issues can be harmonized by using *z-transformation*. A set of measures is z-transformed by centering it (see above) and scaling it by its own standard deviation.
...
By z-transformation, the three scales now exhibit the same mean location and the same dispersion. This could be used to combine them into one general score. Note however, that information is lost by this process, namely the differences in location or dispersion. If the research questions are highly detailed, such as "Is the design consistently rated low on uncanniness?", this can no longer be answered from the z-transformed variable.

Finally, sometimes researchers use *logarithmic transformation* of outcome measures to reduce what they perecive as pathologies of tha data. In particular, many outcome variables do not follow a Normal distribution, as the random term of linear models assumes, but are left-skewed. Log-transformation often mitigates such problems. However, as we will see in chapter [REF GLM], linear models can be estimated gracefully with a random component that precisely matches the data as it comes. The following time-on-task data is from the IPump study, where nurses have tested two infusion pump interfaces:
...
Frequently, it is count measures and temporal measures to exhibit non-symmetric error distributions. The log transformation results in distributions that reasonably  Normal distributed. A the logarithm is a monotonically increasing transformation, the difference between the two conditions is  maintained. However, the natural unit of seconds gets lost by the transformation, making it very difficult to report the results in a quantitative manner.
