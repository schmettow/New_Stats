...
When using the two predictors simultaneously, the overall positive tendency remains. However, we observe major and minor shifts: with `M_3`, the strength of the geekism score is reduced to less than half: `r frm_coef(AUP$T_coef_3, ~model == "M_3", ~fixef == "zgex")`. NCS has shifted, too, but lost only little of its original strength: `r frm_coef(AUP$T_coef_3, ~model == "M_3", ~fixef == "zgex")`. The reason is that the two *predictors are correlated*:
...
We recall the MPM in BrowsingAB: `M_mpm_2` showed a moderate relationship between age and ToT. Another disappointment was the design effect (`M_Design`): a classic statistician may have called it "significant", but only a clown would claim practical relevance. By adding an interaction effect `Design:age_shft` to the model, we will now investigate how differential the designs are.
...
If we can do it with covariates, like age, we can do it with factors, too. For example, does the overall improvement from design A to B depend on education level? Again, we compare the main-effect model to the one with interaction effects. 
...
The reference group in both models is low education level encountering design A. The  effect of design B on low-educated  users is of a similar strength as in the age-by-design model, (`r frm_coef(BrowsingAB$T_ia2, ~model == "M_ia2", ~fixef == "DesignB")`). With design A, education helps a lot: the steps to low and high are so similar, one could almost call it linear. This is different for design B: 
