...
## relational data modelling: a primer

### Populations as tables
### Variables as attributes
### Observations as relations
...
Designing a study is often considered an art. By using data simulation, you can turn it into an engineering process, almost. Where the data model is the blue print for your future data, simulated data is a functional prototype, that

+ makes you think about the data generating process
+ lets you design your statistical model upfront
+ helps you in planning sample size and repeated measures
+ keeps you busy in the Corona crisis




### A simple factorial model

Imagine a study that poses the following hypothesis:

>>>> People who read a lot are smarter.

To test this hypothesis a sample was collected with 50 non-readers and 50 readers and their IQ was measured.

Formally, this is a study design with two conditions, one outcome measure and a sample from the human population. In order to simulate such data we need a few more preparations:

1. What is the *structure of the data*? Or in other words: which effects shall the data contain? In the case here, we can just say 
   + The population of non-readers has an average IQ of 100
   + Readers IQs are increased by 3 points on average

1. What is the *shape of the measures*. In this case, we know that the IQ test is standardized to produces an approximately Normal  distributed population average of 100, with a standard deviation of 15.

The following code produces such a data set:
...
n_Obs      <- 100 ## must be even number
Conditions  <- c("non_read", "reader")
mu_non_read <- 100
diff_read   <- 3
sd_IQ       <- 15
...
D_IQ <- 
  tibble(1:n_Obs,
         Condition = rep(Conditions, n_Obs/2)) %>% 
  mutate(mu = mu_non_read + (Condition == "reader") * diff_read,
         IQ = rnorm(n_Obs, mu = mu, sd = sd_IQ))
...
M_IQ <- lm(IQ ~ Condition, data = D_IQ)
...
Imagine a study that examines whether people who frequently play video games have an improved reaction time. As a measure for gaming frequency, The study asked participants to indicate the approximate number of hours they have played in the last month. Then their average reaction time is measured using a simple experimental task.

Again, we have to examine structure of data (1) and shape of measures (2):

1. For the association between two metric variables, a linear function can be used, for which we have to specify:
    + The intercept, which is RT of a complete non-gamer (zero hours)
    + The improvement per hour of gaming

2. We have to regard both measures, hours of gaming and RT 
    + hours of gaming is a continuous measure with zero as lower boundary, we can use a Gamma distribution
    + RT is a continuous measure with a positive lower bound, we use the ExGaussian distribution (package brms)
...
mu_gaming    <- 10
var_gaming   <- 5
gam_scale     <- mu_gaming/var_gaming 
gam_shape    <- mu_gaming/gam_scale
slope_gaming <- -10
intcpt_gaming<- 300
exg_sigma    <- 120
exg_beta     <- 80
...
D_game <- 
  tibble(1:n_Obs,
         hrs_gaming = rgamma(n_Obs, 
                             shape = gam_shape,
                             scale  = gam_scale)) %>% 
  mutate(mu = intcpt_gaming + hrs_gaming * slope_gaming,
         RT = brms::rexgaussian(n = n_Obs, 
                                mu = mu, 
                                sigma = exg_sigma, 
                                beta = exg_beta))
...
M_game <- lm(RT ~ 1 + hrs_gaming, data = D_game)
...
The problem with RT and ToT data is that Zero is not a possible outcome, as any task uses up a minimum time to complete. 

Only three-parameter distributions can do the trick of setting skewness separately. So called *exponentially modified Gaussian* (Exgaussian) distributions are convolutions of a Gaussian distribution and exponential distribution and have three parameters, $\mu$, $\sigma$ and rate $\beta$. Very roughly, the Gaussian component controls location and dispersion whereas the exponential part adjusts the skew.  When $\beta$ is large in comparison to $\mu$, the distribution is more left skewed. With this additional degree of freedom we can simulate (and estimate) distributions that are far to the right, have strong dispersion *and* strong skewness.


<!-- But, first of all, data simulation lets us implement the data model, directly, which makes it a great illustration. -->
...
Experimental data is an encounter between participants and experimental stimuli. Consequently, the complete data model will comprise two entity tables, participants and stimuli, and one relationship table for the experiment.  However, depending on the design of the experiment and the nature of stimuli, we can distinguish several variations. These variations regard
...
When participants fill out a rating scale, a human sample encounters a set of items. Therefore, we wish two entity tables and a long encounter table. When responses are collected on paper, the best strategy is to create both entity tables, expand them to an empty encounter table and fill this one in.
...
However, ever more often, researchers use web-based survey software to administer questionnaires and demographic variables are gathered along the way. These systems typically output responses in wide format not unsimilar to the following:

```{r}
colnames(R_quest)
```

The demographic data, such as gender, age and household income are single-valued properties of participants and can be represented by the entity tabler Part. Extracting the entity table from the raw data is straight-forward:
...
There even is a case for going one more level up in abstraction and use an entity table for scales. It frequently  happens that rating scales differ in range (of ordinal response categories). In the Hugme study, NCS responses have been gathered on a seven-bin scale (-3 to 3) and Geek on a five-bin scale (-2 to 2). Sometimes ordinal response are coded from 1 to 5, sometimes 1 to 7 and an analogue scales could range from 0 to 1. It is practical, though, to normalize rating scale responses  during the data cleaning process. So, where to store the range? In `D_scales`:

```{r}
D_Scales <-
  tribble(~Scale, ~min, ~max, ~nbins, 
          "NCS" ,    -3,    3,     7,
          "Geek",    -2,    2,     5)
```

Now, all scale responses can be converted to the interval $[0;1]$ by first joining with `D_Items` and then joining with `D_Scales`. As a result it becomes more convenient to plot the distribution of responses per scale side-by-side:

```{r}
D_quest %>%
  join(D_Items, by = "Item") %>%
  join(D_Scales, by = "Scale") %>%
  filter(abs(response) <=3) %>% 
  mutate(response = (response - min)/(nbins - 1)) %>% 
  ggplot(aes(x = Item, y = response, color = Scale)) +
  geom_violin()
